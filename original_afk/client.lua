...
local function SetLocalAFK(state)
    ...
    if state then
        ...
        if Config.LogToConsole then
            print("[original_afk] Input detektálva → AFK törölve")
        end
    end
end
...
CreateThread(function()
    while true do
        ...
        if DoesEntityExist(ped) and not IsEntityDead(ped) then
            -- GetTimeSinceLastInput() → milliszekundum az utolsó input óta
            local idleMs = GetTimeSinceLastInput()
            local stillTime = idleMs / 1000.0
            ...
            if stillTime >= Config.AFKTimeout then
                ...
                if Config.LogToConsole then
                    ...
                    if now - lastLogTime >= Config.LogInterval then
                        lastLogTime = now
                        print(("[original_afk] Idle: %.0f mp | AFK-ig: %d mp"):format(stillTime, remaining))
                    end
                end
            end
